...
namespace Convai.Scripts.Runtime.Features
{
    ...
    public class NPC2NPCConversationManager : MonoBehaviour
    {
        ...
        [SerializeField] private bool startOnPlayerVicinity = true;
        public Func<string, ConvaiGroupNPCController, bool> RelayInterceptor { get; set; }
        ...
        {
            if (startOnPlayerVicinity && isPlayerNear)
        ...
        {
            RelayMessageInternal(message, sender, false);
        }

        public void RelayMessageWithoutInterception(string message, ConvaiGroupNPCController sender)
        {
            RelayMessageInternal(message, sender, true);
        }

        private void RelayMessageInternal(string message, ConvaiGroupNPCController sender, bool bypassInterceptor)
        {
            ...
            if (!bypassInterceptor && RelayInterceptor?.Invoke(message, sender) == true)
                return;
